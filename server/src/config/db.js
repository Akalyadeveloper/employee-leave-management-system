...
import { markDemoMode } from '../services/demoDb.js'
...
export const connectDB = async () => {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose
      ...
      .then((connection) => {
        logger.info('Database', 'MongoDB connected successfully', { host: connection.connection.host })
        return connection
      })
      .catch((error) => {
        ...
        if (process.env.NODE_ENV !== 'production') {
          markDemoMode()
          logger.warn('Database', 'Starting in demo mode with the local in-memory store')
          return null
        }
